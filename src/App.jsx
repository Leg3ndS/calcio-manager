...
function statsFrom(events,state){
  const s={home:{shots:0,on:0,xg:0,passes:0,completed:0,interceptions:0,fouls:0},
           away:{shots:0,on:0,xg:0,passes:0,completed:0,interceptions:0,fouls:0}};
  let hp=0,ap=0;
  for(const e of events){
    const p=e.payload??{}, side=p.side??(p.teamId===state?.teams?.home?.id?"home":p.teamId===state?.teams?.away?.id?"away":e.actors?.[0]?.startsWith?.("home_")?"home":e.actors?.[0]?.startsWith?.("away_")?"away":null);
    if(e.type==="ENGINE_TICK"){if(p.possession==="home")hp+=p.simulatedSeconds||.1;if(p.possession==="away")ap+=p.simulatedSeconds||.1}
    if(!side)continue;
...
class ErrorBoundary extends Component{
  ...
  render(){if(!this.state.error)return this.props.children;return <div className="fatal"><h1>Calcio Manager</h1><b>Errore dell'interfaccia</b><pre>{this.state.error.stack||this.state.error.message}</pre><button onClick={()=>location.reload()}>Ricarica</button></div>}
}
...
function AppContent(){
  ...
  useEffect(()=>{
    try{
      ...
      const unsub=e.subscribe(ev=>{
        allEvents.current.push(ev);setEvents(v=>[...v,ev].slice(-60));
      ...
      const loop=now=>{const dt=Math.min(100,now-last);last=now;if(phaseRef.current==="playing")e.update(dt);sync();raf=requestAnimationFrame(loop)};
  ...
  return <main className="app">
    ...
    <section className="controls">
      ...
      <div className="speeds">{speeds.map(x=><button key={x} className={clock.speed===x?"active":""} onClick={()=>{engine.current.setSpeed(x);setSpeed(x)}}>{x}x</button>)}</div>
    </section>
